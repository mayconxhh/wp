	...
	<div class="fullscreened">
		...
		while( $get_featured_posts->have_posts() ) : $get_featured_posts->the_post();
			...
			if( !empty ( $fitclub_slider_image ) ): // Only continue if feature image is present
				...
				$fitclub_slide_list       .= '<li><div class="parallax-overlay"></div>'.$fitclub_slider_image.'<div class="slider-caption-wrapper">'.'<div class="tg-container">'.'<div class="caption-title">'.get_the_title().'</div>'.'<div class="caption-sub">'.get_the_excerpt().'</div>'.'<a class="slider-readmore" href="'.esc_url( get_permalink() ).'">'.esc_html__( 'Leer Más', 'fitclub' ).'</a>'.'</div></div></li>';
